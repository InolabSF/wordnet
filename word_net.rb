...
class WordNet
  ...
  def get_relatives(word, lang='jpn')
    relatives = []
    words = get_words(word)
    return relatives if words.empty?

    link_combinations = [['hypo', 'hype'], ['hype', 'hypo']]
    link_combinations.each do |link_combination|

      senses = get_senses(words.first)
      link_combination.each_with_index do |link, depth|
        next unless senses
        senses = get_sys_links_recursive(senses, link, lang, 0)
        next if link_combination.count != depth+1

        senses.each do |sense|
          relative = "#{get_word(sense.wordid).lemma}"
          relatives.push(relative) unless relative == word
        end
      end
    end
    relatives
